...
namespace usbprison.aspnetcore
{
    public class PushNotificationService 
    {
        ...
        private readonly PushServiceStoreService _pushServiceStoreService;

        private readonly ILogger _logger;

        public string PublicKey { get { return _pushClient.DefaultAuthentication.PublicKey; } }

        public PushNotificationService(PushServiceClient pushClient, PushServiceStoreService pushServiceStoreService, ILogger<PushNotificationService> logger)
        {
            _pushClient = pushClient;

            _pushServiceStoreService = pushServiceStoreService;
            _logger = logger;
        }
        ...
        private async Task HandlePushMessageDeliveryExceptionAsync(Exception exception, PushSubscription subscription)
        {
            ...
            else
            {
                ...
                {
                    await _pushServiceStoreService.DiscardSubscriptionAsync(subscription.Endpoint);
